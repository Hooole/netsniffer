...
async function main() {
  ...
  log(`\n📋 准备上传到: https://github.com/${username}/ai-resident`, 'cyan');
  ...
  try {
    ...
    try {
      ...
      if (updateRemote.toLowerCase() === 'y' || updateRemote.toLowerCase() === 'yes') {
        execCommand(`git remote set-url origin https://github.com/${username}/ai-resident.git`, '更新远程仓库地址');
      }
    } catch (error) {
      // 没有配置远程仓库，添加新的
      execCommand(`git remote add origin https://github.com/${username}/ai-resident.git`, '添加远程仓库');
    }
    ...
    log('\n🎉 上传完成！', 'green');
    log('\n📝 下一步操作:', 'cyan');
    log('1. 访问 https://github.com/' + username + '/ai-resident', 'cyan');
