...
import { SecurityScanOutlined, CheckCircleOutlined, ExclamationCircleOutlined } from '@ant-design/icons';
...
export const CertificatePanel: React.FC = () => {
  ...
  const handleOpenFile = async () => {
    try {
      if (!window.electronAPI) return;
      const result = await window.electronAPI.certificate.openFile();
      if (!result.success) {
        message.error(result.message);
      }
    } catch (error) {
      message.error('打开证书文件失败');
    }
  };
